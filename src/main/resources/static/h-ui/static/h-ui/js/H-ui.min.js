...
!function (a) {
    a.Huiloading = {
        show: function (d) {
            if (a(".loading-wrapper").length > 0) {
                a(".loading-wrapper").remove()
            }
            if (d == null) {
                d = ""
            }
            var c = '<div class="loading-wrapper"><div class="loading-content"><i class="iconpic iconpic-loading"></i> <span>' + d + "</span></div></div>";
            a(document.body).append(c);
            var b = a(".loading-wrapper .loading-content").width() + 40;
            a(".loading-wrapper .loading-content").css({"margin-left": -(b / 2) + "px",})
        }, hide: function () {
            a(".loading-wrapper").remove()
        }
    }
...
!function (a) {
    a.fn.HuicheckAll = function (b, d) {
        var c = {checkboxAll: 'thead input[type="checkbox"]', checkbox: 'tbody input[type="checkbox"]'};
        var b = a.extend(c, b);
        this.each(function () {
            var e = a(this);
            var g = e.find(b.checkboxAll);
            var f = e.find(b.checkbox);
            g.on("click", function () {
                var k = g.prop("checked");
                f.prop("checked", k);
                var h = 0, j = [];
                f.each(function () {
            ...
            f.on("click", function () {
                var j = f.length;
                var i = f.prop("checked").length;
                j == i ? g.prop("checked", true) : g.prop("checked", false);
                var h = 0, l = [];
                f.each(function () {
                    if (a(this).prop("checked")) {
                        l.push(a(this).val());
...
function HuiaddFavorite(b) {
    ...
    try {
        ...
        try {
            ...
            $("body").Huimodalalert({content: "加入收藏失败，请使用Ctrl+D进行添加", speed: 2000,})
        }
    }
}
...
!function (a) {
    a.fn.Huimodalalert = function (b, d) {
        var c = {btn: ["确定"], content: "弹窗内容", speed: "0", area: ["400", "auto"],};
        var b = a.extend(c, b);
        this.each(function () {
            var m = a(this);
            var f = 0, k = 0, j = 0, e = 0;
            ...
            var g = '<div id="Huimodalalert" class="modal modal-alert radius" style="width:' + f + ";height:" + k + ";margin-left:" + e + '">' + '<div class="modal-alert-info">' + b.content + "</div>" + '<div class="modal-footer">' + '<button class="btn btn-primary radius">' + b.btn[0] + "</button>" + "</div>" + "</div>" + '<div id="Huimodalmask" class="Huimodalmask"></div>';
            if (a("#Huimodalalert").length > 0) {
                a("#Huimodalalert,#Huimodalmask").remove()
            }
            if (b.speed == 0) {
                a(document.body).addClass("modal-open").append(g);
                a("#Huimodalalert").fadeIn()
            } else {
                a(document.body).addClass("modal-open").append(g);
                a("#Huimodalalert").find(".modal-footer").remove();
                a("#Huimodalalert").fadeIn();
                setTimeout(function () {
                    a("#Huimodalalert").fadeOut("normal", function () {
                        n()
                    })
                }, b.speed)
            }
            var i = m.find(".modal-footer .btn");
            i.on("click", function () {
                a("#Huimodalalert").fadeOut("normal", function () {
                    n()
                })
